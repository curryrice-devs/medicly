...
DO $$ BEGIN
  CREATE TYPE session_status_type AS ENUM ('pending', 'active', 'rejective', 'completed');
EXCEPTION
  WHEN duplicate_object THEN null;
END $$;

-- Create sessions table if it doesn't exist (updated schema)
CREATE TABLE IF NOT EXISTS sessions (
  ...
  due_date DATE,
  ai_evaluation JSONB, -- Changed to JSONB for better querying
  patient_notes TEXT, -- Patient's notes for the doctor
  ...
  previdurl TEXT, -- Original video URL in Supabase bucket
  pose_video_id UUID,
  postvidurl TEXT -- Processed video URL in Supabase bucket
);
...
SELECT * FROM (VALUES
  ...
  ('Arm Circles', 'Circular arm movements to improve shoulder range of motion', 'https://example.com/arm-circles'),
  ('Leg Raises', 'Controlled leg lifting exercises to strengthen hip flexors', 'https://example.com/leg-raises'),
  ('Neck Stretches', 'Gentle neck movements to reduce tension and improve mobility', 'https://example.com/neck-stretches')
